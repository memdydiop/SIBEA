<x-layouts.public :title="__('Expertises')">
 ...
 <a href="{{ route('public.expertises.show', $exp->slug) }}" class="group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-surface shadow-[0_8px_30px_rgba(11,31,51,0.06)] hover:shadow-[0_16px_40px_rgba(11,31,51,0.12)] hover:-translate-y-1 transition-all">
 ...
 <img src="{{ $exp->cover_image }}" alt="{{ $exp->title }}" class="h-full w-full object-cover group-hover:scale-[1.05] transition duration-500">
 ...
 <span class="rounded-full bg-white px-2.5 py-1 text-[11px] font-semibold text-primary-900 shadow-sm">{{ $exp->services_count }} {{ __('services') }}</span>
 </div>
 ...
 <h3 class="font-display text-[17px] font-bold leading-tight text-primary-900 line-clamp-2 group-hover:text-primary-700 transition">{{ $exp->title }}</h3>
 @if($exp->excerpt)
