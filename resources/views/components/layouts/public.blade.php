    ...
    <header class="sticky top-0 z-40 w-full border-b border-primary-900/10 bg-primary-900 text-white">
        <div class="mx-auto flex max-w-[1280px] items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-3 font-display font-bold tracking-tight">
                @if($siteLogo)
                    <img src="{{ $siteLogo }}" alt="{{ \App\Models\SiteSetting::get('site_name', 'SIBEA') }} Logo" class="h-10 w-auto object-contain rounded">
                @else
                ...
                @endif
                <span class="hidden text-xs font-normal text-white/60 md:inline">— {{ __('Bâtir le territoire') }}</span>
            </a>
            @php
            ...
            <nav class="hidden gap-6 text-sm font-medium md:flex">
                @if($headerItems->isNotEmpty())
                    @foreach($headerItems as $navItem)
                        ...
                        @endphp
                        <a href="{{ $navItem->url }}" @if($navItem->target === '_blank') target="_blank" rel="noopener noreferrer" @else target="{{ $navItem->target }}" @endif @class(['text-accent' => $isActive, 'text-white/80 hover:text-accent' => ! $isActive]) @if($isActive) aria-current="page" @endif>{{ $navItem->label }}</a>
                    @endforeach
                @else
                    <a href="{{ route('public.expertises.index') }}" class="text-white/80 hover:text-accent">{{ __('Expertises') }}</a>
                    <a href="{{ route('public.projects.index') }}" class="text-white/80 hover:text-accent">{{ __('Réalisations') }}</a>
                    <a href="{{ route('public.programs.index') }}" class="text-white/80 hover:text-accent">{{ __('Programmes') }}</a>
                    <a href="{{ route('public.posts.index') }}" class="text-white/80 hover:text-accent">{{ __('Actualités') }}</a>
                    <a href="{{ route('public.team.index') }}" class="text-white/80 hover:text-accent">{{ __('Équipe') }}</a>
                @endif
            </nav>
