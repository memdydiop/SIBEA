            ...
            <table class="w-full text-sm">
                <thead class="bg-zinc-50 dark:bg-zinc-800/50 text-zinc-500">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3">{{ __('Programme') }}</th>
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse($this->lots as $lot)
                        <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800/30">
                            ...
                            <td class="px-4 py-3 text-xs">{{ $lot->program?->title ?? '—' }}</td>
                            <td class="px-4 py-3 text-right text-xs">{{ $lot->surface !== null ? number_format((float) $lot->surface, 2, ',', ' ') : '—' }}</td>
                            <td class="px-4 py-3 text-right text-xs">{{ $lot->price !== null ? number_format((float) $lot->price, 2, ',', ' ').' FCFA' : '—' }}</td>
                            ...
                            <td class="px-4 py-3 text-center">@if($lot->is_viabilise)<flux:badge variant="success" size="sm">{{ __('Oui') }}</flux:badge>@else<flux:badge variant="zinc" size="sm">{{ __('Non') }}</flux:badge>@endif</td>
                    ...
                    @empty
                        <tr><td colspan="10" class="px-4 py-8 text-center text-zinc-500">{{ __('Aucun lot.') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
