...
<section class="w-full">
 ...
 <flux:table :paginate="$this->testimonials" container:class="rounded-xl border border-zinc-200 bg-white shadow-sm overflow-hidden">
 <flux:table.columns>
 <flux:table.column>{{ __('Auteur') }}</flux:table.column>
 <flux:table.column>{{ __('Société') }}</flux:table.column>
 <flux:table.column align="center">{{ __('Note') }}</flux:table.column>
 ...
 <flux:table.row>
 <flux:table.cell>{{ $t->author_name }}<div class="text-xs text-zinc-500">{{ $t->role }}</div></flux:table.cell>
 <flux:table.cell>{{ $t->company ?? '—' }}</flux:table.cell>
 <flux:table.cell align="center">{{ $t->rating }}/5</flux:table.cell>
 <flux:table.cell align="center">@if($t->is_active)<flux:badge variant="success" size="sm">{{ __('Oui') }}</flux:badge>@else<flux:badge variant="zinc" size="sm">{{ __('Non') }}</flux:badge>@endif</flux:table.cell>
 ...
 @empty
 <flux:table.row><flux:table.cell align="center" colspan="5">{{ __('Aucun témoignage.') }}</flux:table.cell></flux:table.row>
 @endforelse
 </flux:table.rows>
 </flux:table>
 </div>
